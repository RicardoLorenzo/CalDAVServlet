...
import com.ricardolorenzo.icalendar.MatchType;
            ...
            try {
                if (req.getContentLength() > 0) {
                    ...
                    try {
                        ...
                        logger.debug(XMLReader.prettyPrintNode(document));

                        if ("calendar-query".equals(XMLReader.getLocalName(rootElement))) {
                                ...
                                for (Node n : XMLReader.getAllChildElements(filter, "comp-filter")) {
                                    ...
                                    if (n.hasChildNodes()) {
                                        for (Node nn : XMLReader.getChildElements(n)) {
                                            ...
                                            if ("time-range".equals(XMLReader.getLocalName(nn))) {
                                                Period p = null;
                                            ...
                                            } else if ("prop-filter".equals(XMLReader.getLocalName(nn))) {
                                                String subtype = ((Element) nn).getAttribute("name");
                                                for (Node nnn : XMLReader.getChildElements(nn)) {
                                                    if ("text-match".equals(XMLReader.getLocalName(nnn))) {
                                                    	MatchType matchType = MatchType.EQUALS;
                                                    	if (((Element) nnn).getAttributes().getNamedItem("match-type") != null)
                                                    		matchType = MatchType.getEnum(((Element) nnn).getAttributes().getNamedItem("match-type").getNodeValue());
                                                        processCalendarActions(transaction, path, nnn.getTextContent(),
                                                                XML, type, subtype, matchType);
                                                    }
                                                }
                                            }
                                        }
                        ...
                        } else if ("calendar-multiget".equals(XMLReader.getLocalName(rootElement))) {
                            for (Node calendar : XMLReader.getChildElements(rootElement, "href")) {
                                processGet(req, transaction, calendar.getTextContent(), XML);
                            }
                        }
                    } catch (IOException e) {
                    	...
                    	logger.error("report", e);
                        resp.sendError(CalDAVResponse.SC_INTERNAL_SERVER_ERROR);
					} catch (StorePathLockException e) {
                    	logger.error("report", e);
                        resp.sendError(CalDAVResponse.SC_INTERNAL_SERVER_ERROR);
					} catch (StorePathException e) {
                    	logger.error("report", e);
                        resp.sendError(CalDAVResponse.SC_INTERNAL_SERVER_ERROR);
					}
...
//                    XML.write(resp.getOutputStream());
                    ...
                    logger.debug("REPORT Response:" + XMLReader.prettyPrintNode(content));
                    os.write(content);
                    os.close();
                }
    ...
    private void processPeriodCalendarActions(CalDAVTransaction transaction, String path, Period p, XMLWriter XML,
            String type) throws VCalendarException, StorePathLockException, StorePathException {
    ...
    private void processCalendarActions(CalDAVTransaction transaction, String path, Period p, XMLWriter XML, String type)
            throws VCalendarException, StorePathLockException, StorePathException {
        String href = path + "/calendar.ics";
        StorePath _f = this._store.getPath(href);
        if (_f.exists()) {
            ...
            VCalendar vc = VCalendarCache.getVCalendar(this._store,_f);
    ...
    private void processCalendarActions(CalDAVTransaction transaction, String path, String match, XMLWriter XML,
            String type, String subtype, MatchType matchType) throws StorePathLockException, StorePathException {
        String href = path + "/calendar.ics";
        try {
            this.resource_acl = this._store.getResourceACL(transaction, href);
            ...
            try {
                ...
                if ("VEVENT".equals(type)) {
                    if ("UID".equals(subtype)) {
                        VEvent ve = vc.getVevent(match, matchType);
                        if (ve != null) {
                            printVAction(XML, path, eTag, cTag, vc.getTimeZone(), ve);
                        }
                    }
                    ...
                    if ("UID".equals(subtype)) {
                        VTodo vt = vc.getVtodo(match, matchType);
                        if (vt != null) {
                            printVAction(XML, path, eTag, cTag, vc.getTimeZone(), vt);
                        }
                    }
    ...
    private void processGet(HttpServletRequest req, CalDAVTransaction transaction, String path, XMLWriter XML) throws StorePathLockException, StorePathException  {
        String href = path.substring(0, path.lastIndexOf("/"));
        while (href.endsWith(".ics")) {
            href = href.substring(0, href.lastIndexOf("/"));
        }    	
    	//String href = path.substring(path.lastIndexOf("/"));
    	
        String contextPath = req.getContextPath();
		if (href.startsWith(contextPath )) {
            href = href.substring(contextPath.length() + 1);
        }
        ...
        if (_f.exists()) {
            ...
            try {
                VCalendar vc = VCalendarCache.getVCalendar(this._store, _f);
                try {
                    VEvent ve = vc.getVevent(uid, MatchType.EQUALS);
                    if (ve != null) {
                        printVAction(XML, path, eTag, cTag, vc.getTimeZone(), ve);
                    }
                } catch (VCalendarException e) {
                    VTodo vt = vc.getVtodo(uid, MatchType.EQUALS);
                    if (vt != null) {
                        printVAction(XML, path, eTag, cTag, vc.getTimeZone(), vt);
                    }
                }
