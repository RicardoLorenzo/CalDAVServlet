 ...
 * 
 *         To configure the servlet you need to provide the following servlet
 *         init-param's:
 * 
 *         store - the fully qualified class to the calendar store. Standard
 *         stores are:
 *         <code>com.whitebearsolutions.caldav.store.FileSystemStore</code>
 * 
 *         lazy-folder-creation-on-put - This should be 1 for lazy creation of 0
 *         for immediate creation. Defaults to 0
 * 
 *         default-index-file - the default html file to return if the request
 *         is for a folder rather than an calendar object.
 * 
 *         no-content-length-headers This should be 1 if content length headers
 *         are to be suppressed. Defaults to 0
 * 
 *         instead-of-404 Allows you to define a html page that is displayed
 *         rather than a 404. Possibly this can be a URL to redirect to on the
 *         local system but I'm not certain.
 * 
 *         security-provider - allows you to provide an external security
 *         provider to let CalDAVServlet have access to the currently logged in
 *         user. If no security-provider is specified then the default servlet
 *         provider will be used (e.g. req.getUserPrinciple();
 * 
 *         Pass the fully qualified class to your security provider
 *         implementation.
 * 
 */
...
public class CalDAVServlet extends HttpServlet
{
	...
	{
		logger.debug("Sarting CalDavServlet");
	...
	{
		logger.debug("CalDavServlet:init");
		...
		try
		{
			...
			java.lang.reflect.Constructor<CalDAVStore> _c = (Constructor<CalDAVStore>) Class.forName(
					conf.getInitParameter("store")).getConstructor(new Class[]
			{ ServletConfig.class });
			this.store = (CalDAVStore) _c.newInstance(new Object[]
			{ conf });
		...
		{
			logger.error("class=" + initParameter, e);
			throw new ServletException("java class cannot be loaded [" + initParameter + "]: " + e.toString());
		}
		...
		logger.debug("CalDavServlet:complete");
	}
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
	{
		...
		try
		{
			...
			try
			{
				...
				logger.debug("executing method=" + method.toString());
	...
	/**
	 * Instantiates a Calendar Provider from the servlet parameter
	 * security-provider
	 * 
